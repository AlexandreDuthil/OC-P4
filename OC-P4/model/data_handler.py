...
class DataHandler:
        ...
        if isinstance(object, list):
            if isinstance(object[0], Player):
                ...
                for player in object:
                    players_list.append({"last_name": player.last_name, "first_name": player.first_name,
            ...
            if isinstance(object[0], Tournament):
                ...
                for tournament in object:
                    tournament_list.append({"name": tournament.name, "place": tournament.place, "date": tournament.date,
                                            ...
                                            "players": DataHandler.serializer(tournament.players),
                                            "timing_style": tournament.timing_style,
                                            "description": tournament.description,
                                            "round_number": tournament.round_number,
                                            "rounds": DataHandler.serializer(tournament.rounds)})
            ...
            if isinstance(object[0], Round):
                ...
                for round in object:
                    round_list.append({"name": round.name, "starting_date": round.starting_date,
                                       ...
                                       "match_list": DataHandler.serializer(round.match_list),
                                       "ending_date": round.ending_date, "ending_time": round.ending_time})
                return round_list
            if isinstance(object[0], Match):
                match_list = []
                for match in object:
                    match_list.append({"player1": DataHandler.serializer(match.player1),
                                       "player2": DataHandler.serializer(match.player2),
                                       "result": match.result})
                return match_list
    ...
    def player_deserializer(object):
        ...
        if isinstance(object, list):
            for player in object:
                this_player = (Player(player["last_name"], player["first_name"], player["birthdate"],
                                           player["sex"], int(player["rating"])))
                this_player.score = player["score"]
                players_list.append(this_player)
        if isinstance(object, Player):
            this_player = (Player(object["last_name"], object["first_name"], object["birthdate"],
    ...
    def tournament_deserializer(object):
        tournament_list = []
        for tournament in object:
            this_tournament = Tournament(tournament["name"], tournament["place"], tournament["date"],
                                         tournament["tours"], DataHandler.player_deserializer(tournament["players"]),
                                         tournament["timing_style"], tournament["description"],
                                         tournament["round_number"])
            this_tournament.rounds = DataHandler.round_deserializer(tournament["rounds"])
    ...
    def round_deserializer(object):
        ...
        for round in object:
            ...
            this_round.match_list = DataHandler.match_deserializer(round["match_list"])
    ...
    def match_deserializer(object):
        match_list = []
        for match in object:
            this_match = (Match(DataHandler.player_deserializer(match["player1"]),
                                DataHandler.player_deserializer(match["player2"])))
    ...
    def search_tournament(name, place, date):
        return tournament_table.search(Searcher.name == name and Searcher.place == place and Searcher.date == date)
    ...
    def check_existance(object, info2=None, info3=None):
        if isinstance(object, Player):
            ...
            return True if DataHandler.search_player(object, info2, info3) else False
